...
exports.newRegistration = async (req, res, next) => {
  ...
  try {
    ...
    if (foundUser) {
      return res.send({
        message: "the email has been already registerd",
    ...
    res
      .status(500)
      .send({ message: `Please check each forms`, error: "error", err });
  }
};
...
exports.userLogin = async (req, res, next) => {
  ...
  if (email === "" || password === "") {
    return res
      .status(401)
      .send({ message: "Please type required fields", error: "input" });
  }
  try {
    ...
    if (!existingUser) {
      return res
        .status(401)
        .send({ message: "No user found", error: "404 not found" });
    }
    ...
    if (!correctPassword) {
      return res
        .status(401)
        .send({ message: "Invalid credentials", error: "invalid credentials" });
    }
...
exports.verifyUser = async (req, res, next) => {
  ...
  if (!foundPToken) return res.send("params_token not found");
